(source gnu)
(source melpa)
(source marmalade)

(depends-on "cask")

(depends-on "ace-jump-mode")
(depends-on "all")
(depends-on "all-ext")
(depends-on "android-mode")
(depends-on "auto-async-byte-compile")
(depends-on "auto-complete")
(depends-on "auto-complete")
(depends-on "auto-complete-clang")
(depends-on "auto-complete-clang")
(depends-on "auto-complete-clang-async")
(depends-on "auto-complete-etags")
(depends-on "auto-complete-nxml")
(depends-on "auto-complete-octave")
(depends-on "auto-complete-verilog")
(depends-on "auto-indent-mode")
(depends-on "bind-key")
(depends-on "bm")
(depends-on "browse-kill-ring")
(depends-on "c-eldoc")
(depends-on "calfw")
(depends-on "calfw-gcal")
(depends-on "cl-lib")
(depends-on "clojure-mode")
(depends-on "cmake-mode")
(depends-on "coffee-mode")
(depends-on "color-theme")
(depends-on "color-theme-active")
(depends-on "color-theme-actress")
(depends-on "color-theme-blackboard")
(depends-on "color-theme-buffer-local")
(depends-on "color-theme-colorful-obsolescence")
(depends-on "color-theme-complexity")
(depends-on "color-theme-dg")
(depends-on "color-theme-dpaste")
(depends-on "color-theme-eclipse")
(depends-on "color-theme-emacs-revert-theme")
(depends-on "color-theme-github")
(depends-on "color-theme-ir-black")
(depends-on "color-theme-railscasts")
(depends-on "color-theme-sanityinc-solarized")
(depends-on "color-theme-solarized")
(depends-on "color-theme-tango")
(depends-on "color-theme-tangotango")
(depends-on "color-theme-twilight")
(depends-on "color-theme-vim-insert-mode")
(depends-on "color-theme-wombat")
(depends-on "color-theme-wombat+")
(depends-on "color-theme-zenburn")
(depends-on "company")
(depends-on "csharp-mode")
(depends-on "csv-mode")
(depends-on "ctags")
(depends-on "dash")
(depends-on "dash")
(depends-on "deferred")
(depends-on "diff-hl")
(depends-on "doc-mode")
(depends-on "drag-stuff")
(depends-on "ecb")
(depends-on "egg")
(depends-on "el-init")
(depends-on "eldoc-eval")
(depends-on "elscreen")
(depends-on "emacs-eclim")
(depends-on "eproject")
(depends-on "etags-table")
(depends-on "evernote-mode")
(depends-on "exec-path-from-shell")
(depends-on "expand-region")
(depends-on "expand-region")
(depends-on "f")
(depends-on "feature-mode")
(depends-on "findr")
(depends-on "flycheck")
(depends-on "flycheck-cask")
(depends-on "flymake-coffee")
(depends-on "flymake-haml")
(depends-on "flymake-shell")
(depends-on "fold-dwim")
(depends-on "fold-dwim-org")
(depends-on "fringe-helper")
(depends-on "full-ack")
(depends-on "furl")
(depends-on "fuzzy")
(depends-on "fuzzy-match")
(depends-on "gccsense")
(depends-on "gh")
(depends-on "gist")
(depends-on "git-commit")
(depends-on "git-gutter+")
(depends-on "git-gutter-fringe+")
(depends-on "go-mode")
(depends-on "groovy-mode")
(depends-on "guide-key")
(depends-on "haml-mode")
(depends-on "haskell-mode")
(depends-on "haskell-mode")
(depends-on "helm")
(depends-on "helm-ag")
(depends-on "helm-dictionary")
(depends-on "helm-dired-recent-dirs")
(depends-on "helm-flymake")
(depends-on "helm-gist")
(depends-on "helm-gist")
(depends-on "helm-git")
(depends-on "helm-git")
(depends-on "helm-git-grep")
(depends-on "helm-git-grep")
(depends-on "helm-gtags")
(depends-on "helm-ls-git")
(depends-on "helm-ls-hg")
(depends-on "helm-migemo")
(depends-on "helm-open-github")
(depends-on "hideshowvis")
(depends-on "htmlize")
(depends-on "idle-highlight-mode")
(depends-on "inf-ruby")
(depends-on "inflections")
(depends-on "js3-mode")
(depends-on "json")
(depends-on "jump")
(depends-on "levenshtein")
(depends-on "lineno")
(depends-on "linum-off")
(depends-on "logito")
(depends-on "mac-key-mode")
(depends-on "magit")
(depends-on "magit")
(depends-on "markdown-mode")
(depends-on "markdown-mode+")
(depends-on "marmalade")
(depends-on "migemo")
(depends-on "mode-compile")
(depends-on "move-text")
(depends-on "multi-eshell")
(depends-on "multi-term")
(depends-on "multiple-cursors")
(depends-on "multiple-cursors")
(depends-on "mykie")
(depends-on "nyan-mode")
(depends-on "org")
(depends-on "org-email")
(depends-on "org-outlook")
(depends-on "org-table-comment")
(depends-on "org2blog")
(depends-on "page-break-lines")
(depends-on "pager")
(depends-on "pallet")
(depends-on "paredit")
(depends-on "pcache")
(depends-on "popup")
(depends-on "popwin")
(depends-on "popwin")
(depends-on "pos-tip")
(depends-on "powerline")
(depends-on "powershell")
(depends-on "pretty-lambdada")
(depends-on "prodigy")
(depends-on "projectile")
(depends-on "rainbow-delimiters")
(depends-on "redo+")
(depends-on "rinari")
(depends-on "ruby-block")
(depends-on "ruby-compilation")
(depends-on "ruby-electric")
(depends-on "ruby-end")
(depends-on "ruby-mode")
(depends-on "ruby-test-mode")
(depends-on "rw-ispell")
(depends-on "s")
(depends-on "scala-mode")
(depends-on "slime")
(depends-on "slime-repl")
(depends-on "smartparens")
(depends-on "smartrep")
(depends-on "smex")
(depends-on "solarized-theme")
(depends-on "speck")
(depends-on "sr-speedbar")
(depends-on "tabbar")
(depends-on "tabbar")
(depends-on "twittering-mode")
(depends-on "undo-tree")
(depends-on "use-package")
(depends-on "viewer")
(depends-on "w3")
(depends-on "web-mode")
(depends-on "web-mode")
(depends-on "wget")
(depends-on "wikidoc")
(depends-on "xml-rpc")
(depends-on "yaml-mode")
(depends-on "yasnippet")
(depends-on "yasnippet")
(depends-on "zencoding-mode")
