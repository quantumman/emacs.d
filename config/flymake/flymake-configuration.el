...
;; (defvar previous-point 0)
;; (defvar refresh-point 20)

;; (defun adjusted-pos-tip-show (text color)
;;   (setq diff-points (abs (- previous-point (point))))
;;   (if (< refresh-point diff-points)
;;       (progn
;; 	(setq previous-point (point))
;; 	(pos-tip-show text color (point) (selected-window))
;; 	(message ""))
;;     (pos-tip-show text color previous-point (selected-window))
;;     ))
...
;; 	  ;;(showtip (format "[%s] %s" line text))
;; 	  (popup-tip (format "[%s] %s" line text))
;; 	  ;;(pos-tip-show (format "[%s] %s" line text))
;; 	  ;;(adjusted-pos-tip-show (format "[%s] %s" line text) '("white" . "black"))
;; 	  ))
;;       (setq count (1- count)))))
...
;; generate Makefile
(defun flymake-makefile-gen-init (compiler &rest flags)
...
;;   (flymake-init "javac" "-g"))

;; (add-hook 'java-mode
;; 	  '(lambda ()
;; 	     (push '(".+\\.java$" flymake-init) flymake-allowed-file-name-masks)
;; 	     (if (not (null buffer-file-name)) (flymake-mode))))
...
(defvar flymake-objc-compile-options
  (list "-I${HOME}/Library/SDKs/*" "-I/opt/local/include" "-I." "-isysroot" xcode:sdk))
